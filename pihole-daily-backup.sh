...
DATE="$(date +%F_%H%M%S)"
OUT="$BACKUP_DIR/pihole_${DATE}.tar.gz"

SERVICE_PIHOLE="${SERVICE_PIHOLE:-pihole}"
STOP_PIHOLE="${STOP_PIHOLE:-0}"
...
test -d "$BASE/etc-dnsmasq.d" || { echo "Dossier manquant: $BASE/etc-dnsmasq.d"; exit 1; }
test -d "$BASE/etc-pihole" || { echo "Dossier manquant: $BASE/etc-pihole"; exit 1; }

install -d -m 700 -o "$REAL_USER" -g "$REAL_GROUP" "$BACKUP_DIR"
cd "$BASE"

docker compose config --services | grep -qx "$SERVICE_PIHOLE" \
  || { echo "Service '$SERVICE_PIHOLE' introuvable dans docker-compose.yml"; exit 1; }

STOPPED=0
cleanup() {
  if [[ "$STOPPED" -eq 1 ]]; then
    docker compose start "$SERVICE_PIHOLE" >/dev/null 2>&1 || true
  fi
  echo "=== $(date '+%F %T') $(basename "$0") END (duration=$(( $(date +%s) - START_TS ))s) ==="
}
trap 'cleanup' EXIT

if [[ "$STOP_PIHOLE" -eq 1 ]]; then
  docker compose stop "$SERVICE_PIHOLE" >/dev/null 2>&1 || true
  STOPPED=1
fi

items=(etc-pihole etc-dnsmasq.d)
[[ -f "$BASE/pihole.env" ]] && items+=(pihole.env)
[[ -f "$BASE/docker-compose.yml" ]] && items+=(docker-compose.yml)

tar -czf "$OUT" -C "$BASE" \
  --exclude='etc-pihole/gravity_old.db' \
  --exclude='etc-pihole/listsCache/*' \
  --exclude='etc-pihole/pihole-FTL.db' \
  --exclude='etc-pihole/pihole-FTL.db-wal' \
  "${items[@]}"
...
ls -1t "$BACKUP_DIR"/pihole_*.tar.gz 2>/dev/null | tail -n +15 | xargs -r rm -f
