...
DATE="$(date +%F_%H%M%S)"
OUT="$BACKUP_DIR/unbound_${DATE}.tar.gz"

SERVICE_UNBOUND="${SERVICE_UNBOUND:-unbound}"
STOP_UNBOUND="${STOP_UNBOUND:-0}"
...
docker compose config --services | grep -qx "$SERVICE_UNBOUND" \
  || { echo "Service '$SERVICE_UNBOUND' introuvable dans docker-compose.yml"; exit 1; }

STOPPED=0
cleanup() {
  if [[ "$STOPPED" -eq 1 ]]; then
    docker compose start "$SERVICE_UNBOUND" >/dev/null 2>&1 || true
  fi
  echo "=== $(date '+%F %T') $(basename "$0") END (duration=$(( $(date +%s) - START_TS ))s) ==="
}
trap 'cleanup' EXIT

if [[ "$STOP_UNBOUND" -eq 1 ]]; then
  docker compose stop "$SERVICE_UNBOUND" >/dev/null 2>&1 || true
  STOPPED=1
fi
...
ls -1t "$BACKUP_DIR"/unbound_*.tar.gz 2>/dev/null | tail -n +15 | xargs -r rm -f
